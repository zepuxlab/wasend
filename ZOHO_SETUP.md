...
### 3.1. Где найти сообщения (Notes)

**Важно:** Модуль Messages в Zoho CRM работает только через официальные интеграции (WhatsApp Business API, Telegram и т.д.), а не через API. Поэтому сообщения сохраняются в **Notes** лида.

**Где найти Notes:**
1. Откройте Zoho CRM
2. Перейдите в модуль **Leads** (Лиды)
3. Откройте нужного лида (по номеру телефона)
4. В карточке лида найдите вкладку **Notes** (Заметки)
5. Там будут все сообщения WhatsApp с:
   - Направлением (Inbound/Outbound)
   - Текстом сообщения
   - Временем отправки
   - Ссылкой на диалог в Zoho

**Альтернативный способ:**
- В карточке лида также есть раздел **Activities** (Активности), где могут отображаться сообщения
- Но основной способ просмотра - это вкладка **Notes**
...
2. Отправьте тестовую рассылку или используйте тестовый endpoint:
   ```bash
   curl -X POST "https://office.ampriomilano.com/wasend/api/webhook/test-incoming" \
...
3. Проверьте в Zoho CRM:
   - Откройте модуль **Leads** (Лиды)
   - Найдите лида по номеру телефона
   - Откройте карточку лида
   - Перейдите на вкладку **Notes** (Заметки)
   - Там должно быть сообщение с текстом, временем и ссылкой на диалог
